...
- [x] 15.4 Publish: enable GitHub Pages on `docs/` of the default branch and verify the live URL loads the map with its tiles over HTTPS, that a shared geography link works from a cold load, and that the published page logs no console error and makes no cross-origin request other than tiles.
